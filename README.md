...
| URL | 内容 |
| --- | --- |
| `/` | ホーム(ヒーロー、お知らせ、ABOUT + バンド写真、最新リリース、楽曲、ライブ、日記、フォロー) |
| `/profile` | プロフィール(コンセプト + 集合写真、メンバー写真カード、沿革) |
| `/music` | 配信作品(Apple Music 直リンク)/ MV 埋め込み / 曲解説一覧。`/music/[slug]` は楽曲詳細 |
| `/live` | ライブ情報(今後の予定 + 年別アーカイブ) |
| `/diary` | 日記一覧 / `/diary/[slug]` 記事(Markdown対応) |
| `/contact` | コンタクトフォーム(Supabase 受信箱に保存)+ X / 旧フォームへのリンク |
| `/admin` | 管理画面(要ログイン)— お知らせ / 日記 / リリース / 楽曲 / ライブ / 受信箱 の作成・編集・削除 |
...
2. ダッシュボードの **SQL Editor** を開き、次の3ファイルを順に貼り付けて実行:
   1. `supabase/migrations/20260816000000_init.sql`(テーブルとRLSポリシー)
   2. `supabase/migrations/20260816150000_releases.sql`(リリーステーブル + 楽曲の配信リンク列)
   3. `supabase/seed.sql`(初期コンテンツ。旧サイトの情報 + 配信3作品 + サンプル記事)
...
- **リリース**は Apple Music のアルバムURLを設定するとカードから直接飛べます。
  発売日が最も新しい作品がホームの「LATEST RELEASE」に自動で載ります
- **楽曲に YouTube URL を設定すると MV が自動で埋め込まれ**、/music の MOVIE 欄にも並びます
- ライブは日付が今日以降なら自動で **UPCOMING** 扱いになりホームにも表示されます
- コンタクトフォームの受信は管理画面の「受信箱」へ。返信は記載のメールアドレス宛にどうぞ
- 写真の差し替えは `public/images/`(メンバー: `members/`、ジャケット: `releases/`)に置いて
  パスを合わせるだけ
